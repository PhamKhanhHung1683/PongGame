	...
	if (current_gamemode == GM_GAMEPLAY) {
		...
		if (pressed(BUTTON_ENTER)) {
			if (hot_button == EXIT) {
				...
				enemy_is_ai = hot_button ? SINGLE_PLAYER : MULTIPLAYER;
			}
		}
		...
		if (pressed(BUTTON_LEFT)) {
			...
			hot_button = MULTIPLAYER;
		}
		else if (pressed(BUTTON_DOWN)) {
			hot_button = EXIT;
		}

		if (hot_button == SINGLE_PLAYER) {
			draw_text("SINGLE PLAYER", -80, -10, 1, 0xff0000);
			draw_text("MULTIPLAYER", 20, -10, 1, 0xaaaaaa);
			draw_text("EXIT", -10, -30, 1, 0xaaaaaa);
		}
		else if (hot_button == MULTIPLAYER) {
			draw_text("SINGLE PLAYER", -80, -10, 1, 0xaaaaaa);
			draw_text("MULTIPLAYER", 20, -10, 1, 0xff0000);
			draw_text("EXIT", -10, -30, 1, 0xaaaaaa);
		}
		else if (hot_button == EXIT) {
			draw_text("SINGLE PLAYER", -80, -10, 1, 0xaaaaaa);
			draw_text("MULTIPLAYER", 20, -10, 1, 0xaaaaaa);
			draw_text("EXIT", -8, -30, 1, 0xff0000);
		}

		draw_text("PONG GAME", -48, 40, 2, 0xffffff);
	}
}